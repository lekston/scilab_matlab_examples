...
Pr_cov = eye(4,4)*R2/10;
Pr_cov_est = Pr_cov;

//since measurement is limited to bank angle and yaw rate; the measurement matrix H != Id
H = [0 1 0 0;
     0 0 0 1];
...
for i = 1:(size(t,2)-1)
    ...
    //noisy measurement
    z(:,i+1) = H*resp(:,i+1) + v(:,:,i+1)*D; //2x2

    
    //Innovation
    Inn(:,i+1) = z(:,i+1) - H*X_est(:,i+1); //2x1
    
    //Covariance of the innovation
    S(:,:,i+1) = R + H*Pr_cov_est(:,:,i+1)*H'; //2x2
    
    //update Kalman gain
    K(:,:,i+1) = Pr_cov_est(:,:,i+1)*H'*inv(S(:,:,i+1)); //4x2
